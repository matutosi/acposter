...
- 2026-09-02 08:40 (このセッション，x280-home) その17
...
- 2026-09-02 07:55 (このセッション，x280-home) その16
...
**【点検 2026-09-02】バグ8件・文書の食い違い2件・テストの穴3件は片づいた**
(上の その15・16・17)．**残りはリファクタリング3件とテスト1件だけ**．
どれも急がない (**いま壊れているものは無い**)．

- **リファクタリング (3)**: lua の `is_map`・`to_num` が `Pandoc()` の中で毎回
  定義されている → top-level へ出す (テストからも触れるようになる)．
- **リファクタリング (4)**: `by_name`/`matched` の構築が grid 節と layout 節で重複．
- **リファクタリング (5)**: `Get-FrontMatter` を同じファイルに2回呼んでいる
  (`-Md` を省いたとき)．
- **テスト (b)**: ps1 の純関数の Pester テスト．`Resolve-Setting`・`Get-FrontMatter`・
  `ConvertTo-FileUrl` は 2026-09-02 に関数として切り出したので，
  **`poster_common.ps1` に出せば seam ができる**．いまは CI の異常系1件で
  代用している (壊れた md で非0終了することだけ見る)．
