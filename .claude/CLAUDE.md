...
- 2026-08-29 11:30 (このセッション，x280-home)
  **CI (macOS) の失敗ログから，実バグを2件特定して直した**．
  タイムアウト追加 (下記) のおかげで120秒で打ち切られ，「本当にハングしている」
  (パイプ詰まりのような見かけ上の停止ではない) ことが確定した．ログを読み込むと，
  (1) **`file://` URL の組み立て**: `'file:///' + パス` の手組みだと，Mac/Linux の
  絶対パスは元から `/` で始まるため `file:////...` と2重スラッシュになり Chrome が
  読めず固まっていた → `([Uri]$html).AbsoluteUri` に置き換えて解消．
  (2) **画像が埋め込まれない**: `--resource-path` を指定していなかったため，md 内の
  相対パス (`../images/...`) が pandoc の起動時カレントディレクトリ基準で解決され，
  CI (リポジトリ直下から実行) では見つからなかった (Ubuntu でも同じ警告が出ていたが
  致命的にならず気づきにくかった) → `--resource-path=$dir` を追加して解消．
  Windows での回帰確認は済み (両見本とも警告無く1ページ)．CI での再検証はこのあと．
